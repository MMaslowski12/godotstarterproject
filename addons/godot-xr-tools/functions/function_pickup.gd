...
func _process(delta):
	...
	if !enabled or !_controller.get_is_active():
		#print("function_pickup: still not active: ", _controller.get_is_active(), " ", _controller)
		return
...
func _get_closest_grab() -> Node3D:
	...
	#print("SCANNING FOR OBJECTS IN GRAB AREA")
	for o in _object_in_grab_area:
		#print("object in grab area: ", o)
...
func _get_closest_ranged() -> Node3D:
	...
	#print("SCANNING FOR OBJECTS IN GRAB AREA, RANGED")
	for o in _object_in_ranged_area:
		#print("object in grab area ranged: ", o)
		# skip objects that can not be picked up
		if not o.can_pick_up(self):
			continue
